...
function takeLocation(data, instance) {
  ...
  fetchNearestWafasalaf(data.coords.latitude, data.coords.longitude)
    ...
    .catch(error => {
      ...
      instance.send({
        input: {
          text: "Je n'ai pas pu trouver d'agence CIH à proximité. J'ai partagé ma localisation actuelle."
        },
        context: {
          "skills": {
            "actions skill": {
              "skill_variables": {
                ...
                "Directions_Origin": originCoords,
                "Directions_Destination": "Donne-moi+la+CIH+Bank+la+plus+proche+du+point+de+départ",
